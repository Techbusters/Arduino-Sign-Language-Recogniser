...
def initialize_parameters():
    ...
    W1 = tf.get_variable("W1",[4,4,3,8],dtype=tf.float32,initializer=W1)
    W2 = tf.contrib.layers.xavier_initializer(seed = 0)
    W2 = tf.get_variable("W2",shape=[2,2,8,16],dtype=tf.float32,initializer=W2)
    ### END CODE HERE ###
...
T = tf.placeholder(tf.float32,shape=[3])
SQ = T+5;
QD = tf.square(SQ);
with tf.Session() as ss:
    print(ss.run(QD,{T:[1,2,3]}))
